...
| Script | Description |
|--------|-------------|
| **Codex Test** | Connects to a running Codex app-server via WebSocket, sends model context with a prompt, and logs the AI response. Requires `codex app-server --listen ws://127.0.0.1:19000` running. |
| **Codex Plan Apply** | Sends a natural-language change request to a Codex app-server with model context, receives a schema-validated structured plan (11 operations: create/rename/delete elements, create/delete relationships, set documentation/properties, create views, add elements to views, move to folders), previews actions, and applies them on user confirmation. Auto-connects relationships on views. Requires `codex app-server --listen ws://127.0.0.1:19000` running. |
