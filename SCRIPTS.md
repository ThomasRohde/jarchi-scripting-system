...
| **Codex Plan Apply** | Sends a natural-language change request to a Codex app-server with model context, receives a schema-validated structured plan (rename, set property, create relationship), previews actions, and applies them on user confirmation. Requires `codex app-server --listen ws://127.0.0.1:19000` running. |
